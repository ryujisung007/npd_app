...
def run():
    ...
    if st.button("📊 전략 분석 실행"):
        ...
        # AI 전략 보고서
        # ============================================================
        # openai_enabled: secrets에 키가 있고 OpenAI 라이브러리가 설치된 경우
        try:
            openai_enabled = "openai" in st.secrets and bool(st.secrets["openai"].get("OPENAI_API_KEY")) and OpenAI is not None
        except Exception:
        ...
        if openai_enabled:
            ...
            with st.spinner("AI 전략 보고서 생성 중..."):
                ...
                시장 경쟁구조, 가격 전략, 유망 플레이버 방향, 신규 진입 전략을 포함하세요.
                """
